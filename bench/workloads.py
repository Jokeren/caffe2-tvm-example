...
def get_workloads(config):
    ...
    if config == "simple_standard":
        ...
        # conv dw / s1
        workloads.append(Workload(space=112, input_channel=32, output_channel=32, pad=1, stride=1, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=112, input_channel=32, output_channel=64, pad=0, stride=1, kernel=1))
        # conv dw / s2
        workloads.append(Workload(space=112, input_channel=64, output_channel=64, pad=1, stride=2, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=56, input_channel=64, output_channel=128, pad=0, stride=1, kernel=1))
        # conv dw / s1
        workloads.append(Workload(space=56, input_channel=128, output_channel=128, pad=1, stride=1, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=56, input_channel=128, output_channel=128, pad=0, stride=1, kernel=1))
        # conv dw / s2
        workloads.append(Workload(space=56, input_channel=128, output_channel=128, pad=1, stride=2, kernel=3, depthwise=True))
        # conv s1
        workloads.append(Workload(space=28, input_channel=128, output_channel=256, pad=0, stride=1, kernel=1))
        # conv dw / s1
        workloads.append(Workload(space=28, input_channel=256, output_channel=256, pad=1, stride=1, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=28, input_channel=256, output_channel=256, pad=0, stride=1, kernel=1))
        # conv dw / s2
        workloads.append(Workload(space=28, input_channel=256, output_channel=256, pad=1, stride=2, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=14, input_channel=256, output_channel=512, pad=0, stride=1, kernel=1))
        # 5x
        workloads.append(Workload(space=14, input_channel=512, output_channel=512, pad=1, stride=1, kernel=3, depthwise=True))
        ...
        workloads.append(Workload(space=14, input_channel=512, output_channel=512, pad=0, stride=1, kernel=1))
        # conv dw / s2
        workloads.append(Workload(space=14, input_channel=512, output_channel=512, pad=1, stride=2, kernel=3, depthwise=True))
        # conv / s1
        workloads.append(Workload(space=7, input_channel=512, output_channel=1024, pad=0, stride=1, kernel=1))
        # conv dw / s2
        workloads.append(Workload(space=7, input_channel=1024, output_channel=1024, pad=1, stride=2, kernel=3, depthwise=True))
        # conv / s1
